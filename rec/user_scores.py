...
class Users(VerboseMode):
    ...
    def compute_user_scores(self, train_function, *args, **kwargs):
        ...
        self.actual_user_scores = train_function(self.actual_user_profiles,
                                                 *args, **kwargs)
...
class ActualUserScores(VerboseMode):
    ...
    def __init__(self, num_users=None, item_representation=None,
        ...
        if distribution is None or not isinstance(distribution, Distribution):
            self.distribution = Distribution('norm', non_negative=True, integers=True,
                                                loc=0, scale=100)
            self.log('Initialize default normal distribution with no shape')
        else:
    ...
    def compute_actual_scores(self, item_representation, num_users, distribution=None):
        ...
        if distribution is not None and isinstance(distribution, Distribution):
            self.distribution = distribution
            self.user_profiles = self.distribution.compute(shape=(num_users, num_attr))
    ...
    def expand_items(self, item_representation):
        ...
        if item_representation.shape[1] < self.actual_scores.shape[1]:
            raise ValueError("Wrong shape for item_representation")
